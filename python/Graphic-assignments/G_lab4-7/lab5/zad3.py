...
def keyboard_key_callback(window, key, scancode, action, mods):
    ...
    if key == GLFW_KEY_UP and action == GLFW_PRESS:
        if current_component == 0:
            light_ambient = [min(x + 0.1, 1.0) for x in light_ambient[:-1]] + [1.0]
            glLightfv(GL_LIGHT0, GL_AMBIENT, light_ambient)
        elif current_component == 1:
            light_diffuse = [min(x + 0.1, 1.0) for x in light_diffuse[:-1]] + [1.0]
            glLightfv(GL_LIGHT0, GL_DIFFUSE, light_diffuse)
        else:
            light_specular = [min(x + 0.1, 1.0) for x in light_specular[:-1]] + [1.0]
            glLightfv(GL_LIGHT0, GL_SPECULAR, light_specular)
        print_current_values()
