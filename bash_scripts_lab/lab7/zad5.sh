...
#
# Szukamy najpierw linijki z funkcjami, a potem wyodrębniamy nazwy, a potem usuwamy niepotrzebne spacje
# 
# przy funkcji sed tworzymy "grupę przechwytującą", która jest numerowana od lewej do prawej, od 1,2,..
grep -E 'def[ ]+[a-z_]+[ ]*\(.*' dodatkowe/service.py | sed -E 's/def[ ]+([a-z_]+)[ ]*\(.*/\1/g' | sed -E 's/[[:space:]]*//g '  | sort | uniq 
