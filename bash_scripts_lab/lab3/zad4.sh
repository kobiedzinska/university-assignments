...
for dir in aaa bbb ccc; do
	...
	if [ -d "$dir" ]; then
		# Sprawdzamy czy nie ma dowiązań w ddd do katalogów
		if [ ! -e "ddd/$dir" ]; then
			ln -s "$(realpath --relative-to=ddd "$dir")" "ddd/$dir"
			echo "Tworzę dowiązanie $dir"
		else 
			echo "Nie tworzę dowiązania, już istnieje"
		fi
