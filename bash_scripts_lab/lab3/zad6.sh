...
fileX=$(find ccc -type l)

for item in $fileX; do
	if [ -e "$item" ]; then
		# Chcemy ścieżkę do pliku, na który wskazuje nam dowiązanie
		original=$(readlink -f "$item")

		# Wyświetlamy ścieżkę względem zmiennej HOME
		realpath --relative-to="${HOME}" "$original"
	fi
done
