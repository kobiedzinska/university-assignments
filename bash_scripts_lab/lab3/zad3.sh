...
#
fileX=$(find aaa -type f -executable)
# iterujemy po znalezionych plikach
echo "$fileX"
for item in $fileX; do
	# same nazwy plików
	filename=$(basename "$item")
	
	# Jeśli plik nie istnieje w ddd to tworzymy dowiązanie
	if [ ! -e "ddd/$filename" ] ; then
		ln -s "$(realpath --relative-to=ddd "$item")" "ddd/$filename"
		echo "Dowiazanie stworzone dla $filename z $item"
	fi
done
